...
class FetchPopWindow(object):
    def __init__(self,master):
        top = self.top = Toplevel(master)
        # self.master=master
        self.f1=Label(top,text="Which table would you like to see?")
        self.f1.pack()
        self.f2 = Entry(top)
        self.f2.pack()
        ...
        self.f3 =Button(top,text='OK',command=lambda: self.getalltable(self.f2.get()))
        self.f3.pack()
        self.f4 = Button(top, text='Cancel', command=self.cleanup)
        self.f4.pack()
    ...
    def getalltable(self, table):
        if table in {'books', 'users'}:
            if table == 'books':
                fe = f()
                f1 = fe.query_with_fetchone("SELECT * FROM books")
                bid = f1[0]
                bna = f1[1]
                bis = f1[2]
                w.ScrolledTreeview1.insert("", "end", text=bid, values=(bna, bis))

                # w.ScrolledTreeview1.insert("", i, text=i, values=(nm, vot)), tree.pack()
                self.cleanup()
