# TicTacToe (text based) (Python recipe)
# A text based version of TicTacToe for Python.
#     Difficulty is not yet implemented, however,
#     a computer player has been implemented that
#     cannot be beaten.
#
    ...
    def offensiveStrategy(self, board):
        ...
        if self.tactic == 'center':
            ...
            if len(board.blankSpaces()) == 9:
                ...
                if board.board['CM'] != 'O':
                    if self.lastMove != '':
                        if board.board[horizontalFlip[self.lastMove]] == ' ' and board.board[
                            self.lastMove[0] + 'M'] != 'O':
