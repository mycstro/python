...
def aksearch():
    ...
    if not re.findall(r'Download torrent file', str(cont.content)):
        ...
        table = [[OutColors.BW + str(i + 1) + OutColors.DEFAULT if (i + 1) % 2 == 0 else i + 1,
                  ...
                  OutColors.SEEDER + seeders[i] + OutColors.DEFAULT if (i + 1) % 2 == 0 else OutColors.LG + seeders[
                      i] + OutColors.DEFAULT,
                  OutColors.LEECHER + leechers[i] + OutColors.DEFAULT if (i + 1) % 2 == 0 else OutColors.LR + leechers[
                      i] + OutColors.DEFAULT] for i in range(len(href))]
        print()
