...
class deadlockError(Exception):
    ...
    def __str__(s):
        if s.isdir:
            ...
            final = s.file + s.lockfile_ext
        print
        "couldn't find '" + final + "' for " + s.secs + " seconds, params: " \
...
def lock(file, content=None, lockfile_ext=lock_ext, deadlock_timeout=timeout):
    if isdir(file):
        lockdir(file, content, lockfile_ext, deadlock_timeout)
    else:
        lockfile(file, content, lockfile_ext, deadlock_timeout)


def release(file, lockfile_ext=lock_ext):
    if isdir(file):
        releasedir(file, lockfile_ext)
    else:
        releasefile(file, lockfile_ext)
