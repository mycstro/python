...
class TeSpeed:
    ...
    def __init__(self, server="", numTop=0, servercount=3, store=False, suppress=False, unit=False, chunksize=10240,
                 downloadtests=15, uploadtests=10):
    ...
    def FindBestServer(self):
        ...
        best = self.TestLatency(
            self.Closest([self.config['lat'], self.config['lon']], self.server_list, self.bestServers))
        for server in best:
    ...
    def AsyncRequest(self, url, num, upload=0):
        ...
        for i in range(num):
            ...
            if upload == 1:
                connection['connection'] = Process(target=self.AsyncPost,
                                                   args=(connection['child'], full_url, i, num, d))
            else:
                connection['connection'] = Process(target=self.AsyncGet,
                                                   args=(connection['child'], full_url, i, num, d))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ls', '--list-servers', dest='listservers', nargs='?', default=0, const=10,
                        help='List the servers sorted by distance, nearest first. Optionally specify number of servers to show.')
    ...
    parser.add_argument('-n', '--server-count', dest='servercount', nargs='?', default=1, const=1,
                        help='Specify how many different servers should be used in paralel. (Default: 1) (Increase it for >100Mbit testing.)')
    ...
    parser.add_argument('-cs', '--chunk-size', dest='chunksize', nargs='?', type=int, default=10240,
                        help='Specify chunk size after wich tespeed calculates speed. Increase this number 4 or 5 times if you use weak hardware like RaspberryPi. (Default: 10240)')
