...
import services
from dbMySql.models import Stream
import utilites.reddit as reddit
        ...
        for stream in streams:
            if stream.active:
                ...
                if service.enabled and service.use_in_post:
                    service_handler = services.get_service_handler(service)
                    text = safe_format(formats["stream"], service_name=service.name,
    ...
    if len(episodes) > 0:
        ...
        for episode in episodes:
            ...
            table.append(safe_format(formats["discussion"], episode_num=episode.number, episode_link=episode.link,
                                     episode_score=score if score else ""))
        return "\n".join(table)
    else:
